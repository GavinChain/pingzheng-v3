...
import {pingZhengApiStore} from '../../store/modules/pingZhengApi';
import {fuZhuHeSuanMuStore} from '../../store/modules/fuZhuHeSuan';
...
const isFuZhuHeSuan = computed(()=>{
    return pingZhengModelStore.getPingZhengModel.rows.filter(item => {
          // 部门
...
function exportData2() {
  ...
  for (const i in kjsubs) {
    ...
    for (const j in _this.subs) {
      if (((_this.subs[j].ccode + ' ' + _this.subs[j].ccodepath) == kjsubs[i].kuaiJiKeMuText) || $.trim(kjsubs[i].kuaiJiKeMuText) == '') {
        bol = true;
        break;
      } else {
...
function keyUpSetVal(obj, type) {
  ...
  if (type == 'jieMoney' || type == 'daiMoney') {
    ...
    this.pingZhengModel.rows[jquery(obj).closest('tr').index()][type] = $.trim(jquery(obj).text());
    this.ulHide(obj);
    this.colFeed(obj);
  }
}
...
function textareaEnter(obj, type) {
  if ($.trim(jquery(obj).next().find('.active').text()) == '') {
    //this.pingZhengModel.rows[jquery(obj).closest("tr").index()][type] = $.trim(jquery(obj).next().find(".active").text())
  } else {
    this.pingZhengModel.rows[jquery(obj).closest('tr').index()][type] = $.trim(jquery(obj).next().find('.active').text());
  }
...
function textareaEnter2(obj, type, event) {
  if (jquery(obj).next().css('display') == 'block') {
    this.pingZhengModel.rows[jquery(obj).closest('tr').index()][type] = $.trim(jquery(obj).next().find('.active').attr('ccode')) + ' ' + $.trim(jquery(obj).next().find('.active').attr('ccodepath'));
...
function colFeed(obj) {
  if (jquery(obj).closest('td').index() + 1 != jquery(obj).closest('tr').find('td').length) {
    ...
    if ($.trim(jquery(obj).closest('td').next().find('textarea').val()) == '') {
...
function abstractFindSrows(obj) {
  ...
  for (const i in this.abstracts) {
    ...
    if (this.abstracts[i].accabname.split($.trim(jquery(obj).val())).length > 1) {
      this.abstracts[i].hide = false;
      bol = true;
    } else {
...
function subFindSrows(obj) {
  ...
  jquery(obj).val(jquery(obj).val().replace('\n', ''));
  const arr = [];
  this.subFindSrowsAPI($.trim(jquery(obj).val()));
  setTimeout(function() {
    jquery(obj).closest('td').find('ul').show();
...
function proprowsWatch(newVal, oldName) {
  ...
  for (const i in this.pingZhengModel.rows) {
    ...
    if ($.trim(this.pingZhengModel.rows[i].zhaiYao) != '' || $.trim(this.pingZhengModel.rows[i].kuaiJiKeMuText) != '') {
